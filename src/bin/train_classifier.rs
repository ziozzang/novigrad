...
fn args() -> R<Args> {
    ...
    if a.len() < 5 {
        return Err("usage: train_classifier FEATURES INPUT_EDGES PLASTIC_EDGES OUTDIR [--epochs N --seed N --lr FLOAT --gain FLOAT --active-fraction FLOAT --mode supervised|reward|shuffled --actions N --patience N --phase tune|full --readout positive|opponent]".into());
    }
    let mut o = Args {
    ...
    if o.epochs == 0 || o.patience == 0 || o.actions < 2 {
        return Err("epochs and patience must be positive; actions >= 2".into());
    }
    Ok(o)
}
fn run() -> R<()> {
        ...
        for &i in &order {
            ...
            match a.mode {
                Mode::Reward => {
                    ...
                    e.reward(action, if action == y { 1.0 } else { -1.0 })?
                }
                _ => e.reward(y, 1.0)?,
            }
        }
    ...
    let json = format!(concat!(
       ...
       a.lr, a.gain, a.active, a.patience, baseline.acc, baseline.ce, best.acc, best.ce, best.prob,
