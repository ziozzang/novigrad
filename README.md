...
The [causal-feedback supplement](reports/neural-link/feedback.html) separates host belief updates, frozen-LM command fidelity, and actuator calibration. On the reused 64 cases, continuous feedback reaches 52/52/45 successes across three adapters; selecting one host-chosen prototype reaches 64/62/64, with one harmed case retained. Same-first host search reaches 64/63/64, so these results support an interface improvement, not extra LM reasoning. The separate 15,360-input count-noise assay exposes an actuator mapping bottleneck. [Methods and portable replay](results/causal-feedback/README.md) · [한국어](reports/neural-link/feedback.ko.html). [Download the verified replay bundle](https://github.com/ziozzang/novigrad/releases/download/v0.1.1/novigrad-feedback-20260918.tar.gz) · [Distribution verification](results/FEEDBACK_DISTRIBUTION_VERIFICATION.json).
