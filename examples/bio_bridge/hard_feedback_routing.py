...
def prepare():
    ...
    write(OUT / "protocol-lock.json", {
        ...
        "predecode_revision": {"from": "results/hard-feedback-routing-predecode-v0",
            "reason": "Before any hard-routing decoding or outcomes, review found that v0 verification lacked complete cached environment/host/hard-prefix replay. V1 adds that guard and reasserts first-command identity.",
            "v0_protocol_sha256": cf.sha(cf.ROOT / "results/hard-feedback-routing-predecode-v0/protocol-lock.json"),
            "v0_notes_sha256": cf.sha(cf.ROOT / "results/hard-feedback-routing-predecode-v0/NOTES.md")},
...
def hard_rows(initial, prototypes, priors, cases, mapping, generate):
    """Shared run/verification path for the complete hard-routing dynamics."""
    if len(initial) != len(priors) or len(initial) != len(cases):
        raise ValueError("initial prefixes, priors, and cases must align")
    with temporary_hard_routing():
        return [cf.rollout(initial[i], prototypes, priors[i], lm.GOALS.index(case["class"]),
                           mapping, MODE, cf.SCENARIOS[SCENARIO], i, generate)
    ...
    for model in cf.MODELS:
        ...
        def generate(prefix, digest, case_index, model=model):
            ...
            if case_index in DEFAULT_INDICES and digest not in old_cache[model]:
                replay_keys[model].add(digest)
            return raw
        rows = hard_rows(initial, prototypes, priors, cases, lock["executor_actions"], generate)
...
def verify():
    ...
    replayed_conditions, replayed_comparisons = {}, {}
    for model in cf.MODELS:
        rows = hard_rows(tensors[model + "/initial"].to("mps"),
                         tensors[model + "/prototypes"].to("mps"),
                         tensors[model + "/prior"].numpy(), cases, lock["executor_actions"],
                         lambda prefix, digest, case_index, model=model: cache[model][digest])
        original, continuous = _original_rows(old_results, model), _continuous_rows(old_results, model)
        _assert_first_identity(rows, original)
        replayed_conditions[model] = {"metrics": cf.summarize(rows), "cases": rows}
        replayed_comparisons[model] = _comparison(rows, continuous)
    if replayed_conditions != saved["conditions"] or replayed_comparisons != saved["comparisons"]:
        raise AssertionError("complete cached environment/host/hard-prefix replay differs")
    counts = {}
    ...
    write(OUT / "verification.json", {"selected_actual_generation_exact": True,
          ...
          "complete_environment_host_hard_prefix_cache_replay_exact": True,
          "same_case_first_raw_command_identity_reasserted": True,
