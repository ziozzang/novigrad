...
class MotorCommand:
 ...
 def __post_init__(self):
  ...
  if self.duration_s<=0 or self.issued_s<0 or self.expires_s<=self.issued_s or self.expires_s>self.issued_s+self.duration_s:raise ValueError('invalid command timing')
...
class CommandReceiver:
 ...
 def accept(self,command,now):
  ...
  if command.issued_s>now or command.expires_s<=now:raise ValueError('stale or future command')
  self.last_sequence=command.sequence_id;self.command=command
 def rate(self,now):
  if not finite(now):raise ValueError('invalid receiver time')
  return 0. if self.command is None or now>=self.command.expires_s else self.command.turn_rate_rad_s

class HeadingTokenCodec:
 """Host-owned discrete IDs, explicitly unrelated to any LM vocabulary."""
 def __init__(self,bins):
  if type(bins) is not int or not 4<=bins<=1000:raise ValueError('bins must be an integer in [4,1000] to keep heading and goal token ranges disjoint')
  self.bins=bins
 ...
 *({'name':f'bins{b}_true_disturbance','bins':b,'feedback':'true_feedback','schedule':'100hz','disturbance':'applied'} for b in (16,64,256)),
 ...
 {'name':'bins64_true_no_disturbance','bins':64,'feedback':'true_feedback','schedule':'100hz','disturbance':'logged_not_applied'},
 {'name':'bins64_actuator_no_effect_disturbance','bins':64,'feedback':'true_feedback','schedule':'100hz','disturbance':'applied','motor_plant_gain':0.})
CONFIG={'dt_s':DT,'rate_hz':100,'steps':STEPS,'duration_s':3.,'goal_switch_s':1.5,'seeds':list(SEEDS),'initial_conditions_per_seed':INITIALS,'controller_gain':2.,'max_turn_rate_rad_s':2.5,'success_error_rad':.15,'command_validity':'half-open [issued_s, expires_s)','conditions':list(CONDITIONS),'anti_goal_boundary':'Both sampled goals are checked only against the initial heading. A goal within .02 rad of exact pi initial error is shifted by .03 rad. There is no switch-time anti-goal guard; all resulting failures are retained.'}
...
def simulate_case(condition,seed,index):
 ...
 for tick in range(STEPS+1):
  ...
  if tick==STEPS:break
  applied_disturbance=disturbance if condition['disturbance']=='applied' else 0.;motor_gain=condition.get('motor_plant_gain',1.);delta=(motor_gain*rate+applied_disturbance)*DT;heading=wrap_angle(heading+delta);action_copy=rate*DT;actual_turn=delta;copy_heading=wrap_angle(copy_heading+action_copy)
...
def run():
 ...
 OUT.mkdir(parents=True);source=Path(__file__);test=source.with_name('test_robot_token_bridge.py');source_hashes={str(p.relative_to(ROOT)):sha(p) for p in (source,test)};config_hash=hashlib.sha256(json.dumps(CONFIG,sort_keys=True,separators=(',',':')).encode()).hexdigest();lock={'config':CONFIG,'config_sha256':config_hash,'source_sha256':source_hashes,'freeze_boundary':'Written before build_report or any corrected outcome calculation'};(OUT/'protocol-lock.json').write_text(json.dumps(lock,indent=2)+'\n');report=build_report();report['provenance']={'source_sha256':source_hashes,'config_sha256':config_hash,'claims':'Interface and deterministic kinematic measurements only; no biological circuit or LM was trained or executed'};(OUT/'results.json').write_text(json.dumps(report,separators=(',',':'),allow_nan=False)+'\n');(OUT/'run-manifest.json').write_text(json.dumps({'protocol_lock_sha256':sha(OUT/'protocol-lock.json'),'results_sha256':sha(OUT/'results.json')},indent=2)+'\n')
def verify():
 lock=json.loads((OUT/'protocol-lock.json').read_text());manifest=json.loads((OUT/'run-manifest.json').read_text())
 if lock['config']!=CONFIG or lock['config_sha256']!=hashlib.sha256(json.dumps(CONFIG,sort_keys=True,separators=(',',':')).encode()).hexdigest():raise ValueError('config lock differs')
 for name,digest in lock['source_sha256'].items():
  if sha(ROOT/name)!=digest:raise ValueError('source changed: '+name)
 if sha(OUT/'protocol-lock.json')!=manifest['protocol_lock_sha256'] or sha(OUT/'results.json')!=manifest['results_sha256']:raise ValueError('locked artifact hash changed')
 ...
 (OUT/'verification.json').write_text(json.dumps({'config_lock_exact':True,'source_hashes_exact':True,'exact_full_trace_replay':True,'cases':len(CONDITIONS)*len(SEEDS)*INITIALS,'results_sha256':manifest['results_sha256']},indent=2)+'\n')
