"""Offline browser checks for the causal feedback study supplement."""
import hashlib
import json
...
OUT = Path('/tmp/novigrad-feedback-report-qa')
...
def main():
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(
            ...
            headless=True)
        for name in ('feedback.html', 'feedback.ko.html'):
            ...
            assert disclosure.locator('table, article').count() > 0
            ...
            page.close()
        browser.close()
    (ROOT / 'feedback-browser-verification.json').write_text(
